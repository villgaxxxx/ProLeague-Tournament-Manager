...
export default function Matches() {
    ...
    useEffect(() => {
        const connection = new signalR.HubConnectionBuilder()
            .withUrl("http://proleague-api.somee.com/matchHub")
    ...
    const handleStartMatch = async (id) => {
        ...
        await fetch(`http://proleague-api.somee.com/api/Matches/${id}/start`, {
    ...
    const handlePostponeMatch = async (id) => {
        ...
        const token = localStorage.getItem('adminToken');
        await fetch(`http://proleague-api.somee.com/api/Matches/${id}/postpone`, {
            method: 'PUT',
    ...
    const actionPlayer = async (matchId, playerId, action) => {
        const token = localStorage.getItem('adminToken');
        await fetch(`http://proleague-api.somee.com/api/Matches/${matchId}/${action}/${playerId}`, {
            method: 'PUT', headers: { 'Authorization': `Bearer ${token}` }
        });
        fetchMatches();
    };
    ...
    const handleFinishMatch = async (id) => {
        ...
        if (isKnockout && isTie) {
            ...
            const token = localStorage.getItem('adminToken');
            const response = await fetch(`http://proleague-api.somee.com/api/Matches/${id}/finish-knockout`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        ...
        const token = localStorage.getItem('adminToken');
        const response = await fetch(`http://proleague-api.somee.com/api/Matches/${id}/finish`, {
            method: 'PUT', headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await response.json();
        if (data.championName || data.ChampionName) {
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex flex-col items-center justify-center text-center p-4 animate-fade-in" dir="rtl">
                    <div className="bg-gradient-to-b from-yellow-400 via-amber-500 to-amber-600 p-1 rounded-3xl shadow-2xl max-w-lg w-full transform scale-100 transition-all animate-bounce-short">
                        <div className="bg-slate-900 text-white rounded-3xl p-8 flex flex-col items-center relative overflow-hidden">
                            ...
                            <button onClick={() => setChampion(null)} className="bg-gradient-to-r from-gray-800 to-gray-700 hover:from-black hover:to-gray-900 text-white font-black px-8 py-3 rounded-xl border border-gray-600 transition shadow-lg w-full">إغلاق والعودة للوحة التحكم ✖️</button>
                        </div>
                    </div>
                </div>
            )}
